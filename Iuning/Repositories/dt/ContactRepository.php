...
class ContactRepository extends BaseDtRepo
{
    ...
    public function __construct()
    {
        ...
        $this->builder = DB::table('contact')->from("contact as c")
                        // ->leftJoin('employee as e','e.id','=','c.create_by')
 ...
 */
    public function dtSetResult($key, $val, $row)
    {
        switch ($key) {
            case 'action' :
                ...
                // if(isset($row['last_contact_datetime']) && $this->todayPersonal){
    ...
    public function SpacialQuery()
    {
        ...
        $date_filter = null;
        $dept_sales_filter = null;

        if(isset($this->allSearch[0])){
            $d = explode("@@",$this->allSearch[0]);
            $date_filter = $d[0];
            $dept_sales_filter = $d[1]??null;

            switch ($date_filter) {
                case 'today':
                    $this->builder->whereBetween('c.next_contact_date',[$today,$today]);
                    break;
                case 'before':
                    $this->builder->where('c.next_contact_date', '<', $today);
                    $this->builder->whereNull('c.contact_time_ed');
                    // $this->builder->whereNull('cus.last_contact_datetime');
                    break;
                case 'after':
                    $this->builder->where('c.next_contact_date', '>', $today);
                    break;
            ...
            if($this->isPersonal && $date_filter == 'today'){
                $this->todayPersonal = true;
            }
        }else{
            if($this->isPersonal){
                $this->todayPersonal = true;
        ...
        if($this->isPersonal){
            return $this->builder->where('c.create_by', Auth::user()->id);//個人
        }else{
            //部門
            if(isset($dept_sales_filter)){
                return $this->builder->where('c.create_by', $dept_sales_filter);//部門的人們
            }else{
                $create_bys = DepartmentEmployee::findLeadingEmployees(Auth::user()->id);
                return $this->builder->whereIn('c.create_by', $create_bys);//部門的人們
            }
        }
    }
